      ...
      tabItems(
        tabItem(tabName = "dashboard",
              ...
              p("In a state of growing knowledge and concern regarding climate change, our app allows users to visualize the direction, 
                ...
                and visualizations such as ours, we will eventually be able to predict the path of future storms based on their origin and past storm conditions."),
              br(),
        ...
        tabItem(tabName = "stormFreq",
            fluidPage(
              ...
              sidebarLayout(
                selectizeInput("stormType", label = "Select Storm Type", choices = unique(globalHurricane$Status), selected = globalHurricane$Status[1], multiple = TRUE),
              sliderInput("stormYear", label = "Year", min = 1851, max = 2015 , value = 1851, sep="" , animate = animationOptions(interval = 200, loop = FALSE))),
              sidebarPanel("We built this tool as a means to show how the frequency and spread of storms has increased over time.", width = 10),
              mainPanel(
                plotOutput("stormBar")))),
